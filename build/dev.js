...
portfinder
  ...
  .then((port) => {
    ...
    Server.listen(ENABLE_PORT, HOST, function() {
      // show toast
      spinner.text =
        Colors.green('Your application is running here:') +
        ' ' +
        Colors.cyan(`http://localhost:${ENABLE_PORT}/${OPEN_URL}`);
    });
  })
  .catch((err) => {
    spinner.text = Colors.red(
      'Your application running failed, as starting port'
    );
